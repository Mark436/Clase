        ...
        <>
          <div className="grid grid-cols-2 gap-4">
            <Card className="flex flex-col items-center gap-1 py-5 text-center">
              ...
              <AverageValue value={alumno?.promedioGeneral} />
            </Card>
            <Card className="flex flex-col items-center gap-1 py-5 text-center">
              <p className="text-xs font-semibold uppercase tracking-wide text-on-surface-variant">
                Del periodo
...
function AverageValue({ value }: { value: number | undefined }) {
  if (value === undefined || !Number.isFinite(value)) {
    return <span className="font-display text-2xl font-bold">—</span>;
  }

  // Averages render with at most two decimals.
  const rounded = Math.round(value * 100) / 100;

  return (
    <AnimatedNumber
      value={rounded}
      decimals={String(rounded).split(".")[1]?.length ?? 0}
      className="font-display text-3xl font-bold tabular-nums text-primary-strong"
    />
  );
}
